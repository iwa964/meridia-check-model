...
def _bad_items(path: str, value: list) -> list:
    ...
    # A group of fewer than two distinct ids joins nothing: a mistyped one would silently group nothing.
    return [v for v in value if not (isinstance(v, list) and all(_is_id(i) for i in v) and len(set(v)) >= 2)]
...
def _check_type(path: str, default, value) -> None:
    ...
    if isinstance(value, list):
        ...
        if bad:
            holds = "non-empty strings" if LIST_ITEMS[path] == "str" else "lists of two or more distinct id strings"
